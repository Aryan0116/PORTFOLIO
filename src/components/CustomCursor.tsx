...
const CustomCursor: React.FC = () => {
  ...
  const updatePosition = useCallback((e: MouseEvent) => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      // Smooth outer ring following with slight delay
      setOuterPosition(prev => ({
        x: prev.x + (newPos.x - prev.x) * 0.1,
        y: prev.y + (newPos.y - prev.y) * 0.1
      }));
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          transform: `scale(${isHovering ? 1.2 : 1}) rotate(${isClicking ? '45deg' : '0deg'})`,
          willChange: 'transform',
        }}
      >
        <div className="w-10 h-10 border-2 border-indigo-500/40 rounded-full"></div>
      </div>
    </>
  );
};
